...
def armar_layout(tarjeta, layout_tarjeta, dificultad_elegida, dataset_elegido, usuario_elegido):
    ...
    columna_izq = [
        [sg.Frame('Dificultad',
                  [[sg.Text(dificultad_elegida, font=cgen.FUENTE_COMBO)]], expand_x=True)],
        [sg.Frame('Tema',
    ...
    columna_centro = [
        [sg.Frame('Tiempo restante', [[sg.Text(f'00:00', key='-JUEGO_TIEMPO-', font=cgen.FUENTE_COMBO),
                                       sg.ProgressBar(datos.get_tiempo(), orientation='h',
    ...
    columna_der = [
        ...
        [sg.Table(values=list(enumerate(resultados, start=1)), headings=['Pregunta', 'Resultados'],
